...
class ProvinceResourceIT {
    ...
    @Test
    @Transactional
    void searchProvince() throws Exception {
        // Configure the mock search repository
        // Initialize the database
        provinceRepository.saveAndFlush(province);
        when(mockProvinceSearchRepository.search(queryStringQuery("id:" + province.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(province), PageRequest.of(0, 1), 1));

        // Search the province
        restProvinceMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + province.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(province.getId().intValue())))
            .andExpect(jsonPath("$.[*].libelle").value(hasItem(DEFAULT_LIBELLE)));
